...
class CreateRoomSerializer(serializers.Serializer):
    ...
    member_ids = serializers.ListField(
        child=serializers.IntegerField(), min_length=1, help_text="List of user IDs to add to room"
    )
...
class ChatRoomCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not is_group and len(member_ids) == 1:
            ...
            existing = ChatRoom.objects.filter(is_group=False, members=request.user).filter(members=other_user).first()

            if existing:
                return Response(
                    ChatRoomSerializer(existing, context={"request": request}).data, status=status.HTTP_200_OK
                )

        # Create room
        room = ChatRoom.objects.create(name=name if is_group else None, is_group=is_group, created_by=request.user)
...
class ChatMessageListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        room = get_object_or_404(ChatRoom.objects.filter(members=self.request.user), id=room_id)
