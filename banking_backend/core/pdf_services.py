...
def generate_generic_report_pdf(title, subtitle, headers, data, summary_data=None):
    ...
    # Report Title
    elements.append(
        Paragraph(f"<b>{title}</b>", ParagraphStyle("ReportTitle", parent=styles["Heading2"], alignment=TA_CENTER))
    )
    if subtitle:
...
def generate_payslip_pdf(payslip):
    ...
    deductions_data = [
        ["Description", "Amount (GHS)"],
        ["SSNIT (5.5%)", f"{payslip.ssnit_contribution:,.2f}"],
...
def generate_statement_pdf(statement, transactions):
    ...
    elements.append(
        Paragraph(
            "<b>ACCOUNT STATEMENT</b>", ParagraphStyle("StatementTitle", parent=styles["Heading2"], alignment=TA_CENTER)
        )
    )
    elements.append(Spacer(1, 5 * mm))
