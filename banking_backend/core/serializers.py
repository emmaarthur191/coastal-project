...
from .models import Account, Transaction, Loan, FraudAlert, BankingMessage, ServiceRequest, AccountOpeningRequest, AccountClosureRequest, MessageThread, Message, Device, Payslip, AccountStatement
...
class AccountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Account
        fields = ['id', 'user', 'account_number', 'account_type', 'account_type_display', 'balance', 'calculated_balance', 'is_active', 'created_at', 'updated_at']
...
class TransactionSerializer(serializers.ModelSerializer):
    class Meta:
        model = Transaction
        fields = ['id', 'from_account', 'to_account', 'amount', 'transaction_type', 'description', 'status', 'timestamp', 'processed_at']
...
class LoanSerializer(serializers.ModelSerializer):
    class Meta:
        model = Loan
        fields = ['id', 'user', 'amount', 'interest_rate', 'term_months', 'outstanding_balance', 'status', 'approved_at', 'created_at', 'updated_at']
...
class BankingMessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'subject', 'body', 'is_read', 'read_at', 'thread_id', 'parent_message', 'replies', 'created_at']
...
class ServiceRequestSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'user', 'request_type', 'description', 'delivery_method',
            'status', 'admin_notes', 'processed_by', 'processed_at', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'user', 'status', 'admin_notes', 'processed_by', 'processed_at', 'created_at', 'updated_at']
...
class RefundSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        model = Refund
        fields = [
            'id', 'user', 'transaction', 'amount', 'reason', 'description',
            'status', 'admin_notes', 'processed_by', 'processed_at', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'user', 'status', 'admin_notes', 'processed_by', 'processed_at', 'created_at', 'updated_at']
...
class ComplaintSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        model = Complaint
        fields = [
            'id', 'user', 'category', 'priority', 'subject', 'description',
            'status', 'resolution', 'assigned_to', 'resolved_by', 'resolved_at', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'user', 'status', 'resolution', 'assigned_to', 'resolved_by', 'resolved_at', 'created_at', 'updated_at']
...
class AccountOpeningRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'account_type', 'card_type', 'id_type', 'id_number',
            'first_name', 'last_name', 'date_of_birth', 'nationality', 
            'address', 'phone_number', 'email', 'photo', 'status', 
            'processed_by', 'submitted_by', 'created_account',
            'rejection_reason', 'notes', 'created_at', 'updated_at', 'approved_at'
        ]
        read_only_fields = [
            'id', 'status', 'processed_by', 'submitted_by', 'created_account',
            'rejection_reason', 'notes', 'created_at', 'updated_at', 'approved_at'
        ]
...
class AccountClosureRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'account', 'account_id', 'closure_reason', 'other_reason',
            'phone_number', 'otp_verified', 'status', 'processed_by', 'submitted_by',
            'rejection_reason', 'notes', 'final_balance', 'created_at', 'updated_at', 'closed_at'
        ]
        read_only_fields = [
            'id', 'account', 'status', 'processed_by', 'submitted_by',
            'rejection_reason', 'notes', 'final_balance', 'created_at', 'updated_at', 'closed_at'
        ]
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'content', 'encrypted_content', 'iv', 'auth_tag', 'message_type',
            'is_system_message', 'attachment_url', 'attachment_name',
            'created_at', 'edited_at', 'is_read_by_me'
        ]
...
class MessageThreadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MessageThread
        fields = [
            'id', 'subject', 'thread_type', 'participants', 'participant_list',
            'participant_ids', 'created_by', 'created_by_name', 'is_archived', 
            'is_pinned', 'last_message_at', 'created_at', 'updated_at',
            'messages', 'unread_count', 'last_message_preview'
        ]
...
class DeviceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Device
        fields = [
            'id', 'user', 'device_token', 'device_type', 'device_name',
...
class CashAdvanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = CashAdvance
        fields = [
            'id', 'user', 'user_name', 'amount', 'reason', 'status',
            'approved_by', 'approved_by_name', 'repayment_date', 'repaid_at',
...
class CashDrawerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = CashDrawer
        fields = [
            'id', 'cashier', 'cashier_name', 'drawer_number', 'opening_balance',
            'current_balance', 'closing_balance', 'expected_balance', 'variance',
            'status', 'opened_at', 'closed_at', 'notes', 'denominations',
...
class CheckDepositSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = CheckDeposit
        fields = [
            'id', 'account', 'account_number', 'amount', 'check_number',
            'bank_name', 'routing_number', 'front_image', 'back_image',
            'status', 'rejection_reason', 'processed_by', 'processed_by_name',
            'created_at', 'processed_at', 'cleared_at'
        ]
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = Product
        fields = [
            'id', 'name', 'product_type', 'product_type_display', 'description',
            'interest_rate', 'minimum_balance', 'maximum_balance', 'features',
            'terms_and_conditions', 'is_active', 'created_at', 'updated_at'
        ]
...
class PromotionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = Promotion
        fields = [
            'id', 'name', 'description', 'discount_percentage', 'bonus_amount',
            'start_date', 'end_date', 'is_active', 'is_currently_active',
            'eligible_products', 'eligible_product_names', 'terms_and_conditions',
            'max_enrollments', 'current_enrollments', 'created_at', 'updated_at'
        ]
...
class ReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = Report
        fields = [
            'id', 'template', 'template_name', 'title', 'report_type',
            'format', 'format_display', 'status', 'status_display',
            'file_url', 'report_url', 'file_path', 'file_size', 'generated_by',
            'generated_by_name', 'parameters', 'error_message',
            'created_at', 'completed_at'
        ]
        read_only_fields = ['id', 'generated_by', 'file_url', 'file_path', 'file_size', 'error_message', 'created_at', 'completed_at']
...
class ReportScheduleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'day_of_week', 'day_of_month', 'time_of_day', 'format', 'parameters',
...
class SystemHealthSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'response_time_ms', 'details', 'checked_at'
        ]
...
class UserMessagePreferencesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'sound_enabled', 'notification_sound',
            'read_receipts_enabled', 'typing_indicators_enabled', 'last_seen_visible',
            'auto_delete_enabled', 'auto_delete_days',
            'markdown_enabled', 'emoji_shortcuts_enabled', 'font_size',
...
class BlockedUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = BlockedUser
        fields = [
            'id', 'blocker', 'blocked', 'blocker_username', 'blocked_username',
            'blocked_full_name', 'reason', 'created_at'
        ]
...
class ClientAssignmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'mobile_banker', 'mobile_banker_name', 'client', 'client_name',
            'location', 'status', 'status_display', 'amount_due', 'amount_due_formatted',
            'next_visit', 'next_visit_formatted', 'priority', 'priority_display',
...
class PayslipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payslip
        fields = [
            'id', 'staff', 'staff_name', 'staff_id_display', 'month', 'year', 'month_name',
            'pay_period_start', 'pay_period_end',
            'base_pay', 'allowances', 'overtime_pay', 'bonuses', 'gross_pay',
            'ssnit_contribution', 'tax_deduction', 'other_deductions', 'total_deductions',
            'net_salary', 'pdf_file', 'download_url',
            'generated_by', 'generated_by_name', 'notes', 'is_paid', 'paid_at',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'gross_pay', 'ssnit_contribution', 'total_deductions', 'net_salary', 'created_at', 'updated_at']
...
class AccountStatementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'account', 'account_number', 'account_type_display',
        ...
        read_only_fields = ['id', 'transaction_count', 'opening_balance', 'closing_balance', 'created_at', 'generated_at']
