...
def get_cash_flow(request):
    ...
    # Deposits (positive deposit transactions)
    ...
    # Loan Repayments (money coming back from loans)
    ...
    # Service Charges & Fees (from fee transactions)
    ...
    # Transfers In (incoming transfers)
    ...
    # Commissions earned (if tracked separately)
...
def get_mobile_banker_metrics(request):
    ...
    # Scheduled Visits - visits assigned to this mobile banker that are scheduled or in progress
    scheduled_visits = VisitSchedule.objects.filter(
        assigned_to=user,
        status__in=['scheduled', 'in_progress']
    ).count()

    # Completed Today - visits completed today
    ...
    # Collections Due - outstanding loan repayments due
    ...
    # New Applications - loan applications submitted today
