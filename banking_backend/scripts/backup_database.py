#!/usr/bin/env python3
"""
Database Backup Script for Banking Backend
Creates automated backups of the PostgreSQL database with encryption and compression.
"""
...
class DatabaseBackup:
    """Handles database backup operations with encryption and compression."""

    def __init__(self, backup_dir=None, encryption_key=None):
        self.backup_dir = Path(backup_dir or settings.BACKUP_DIR or 'backups/database')
        ...
        self.db_name = db_settings.get('NAME', 'banking_db')
        self.db_user = db_settings.get('USER', 'banking_user')
        self.db_password = db_settings.get('PASSWORD', 'banking_password')
    ...
    def create_backup(self, filename=None):
        ...
        try:
            ...
            raise CommandError(f"Failed to create backup: {e.stderr}")
    ...
    def restore_backup(self, backup_path, confirm=True):
        ...
        try:
            ...
            raise CommandError(f"Failed to restore backup: {e.stderr}")
    ...
    def list_backups(self):
        ...
        for file_path in self.backup_dir.glob('*'):
            if file_path.is_file() and ('backup' in file_path.name or file_path.suffix in ['.sql', '.gz', '.enc']):
                stat = file_path.stat()
                backups.append({
                    'name': file_path.name,
                    'path': file_path,
...
if __name__ == '__main__':
    ...
    command = Command()
    parser = argparse.ArgumentParser(description='Database backup operations')
