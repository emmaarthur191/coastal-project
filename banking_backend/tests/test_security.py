...
def account_a(db, user_a):
    ...
    return Account.objects.create(
        user=user_a, account_number="2231000000001", account_type="daily_susu", balance=Decimal("1000.00")
    )
...
def account_b(db, user_b):
    ...
    return Account.objects.create(
        user=user_b, account_number="2231000000002", account_type="daily_susu", balance=Decimal("500.00")
    )
    ...
    def test_concurrent_withdrawal_prevents_double_spend(self, user_a, account_a, staff_user):
        ...
        def withdraw():
            ...
            return client.post(
                ...
                {"member_id": user_a.id, "amount": "600.00", "type": "withdrawal", "account_type": "daily_susu"},
            )
        ...
        assert len(successes) <= 1, "Double spend occurred! Both transactions succeeded."
...
class TestUnauthorizedAccess:
    ...
    def test_user_cannot_access_other_user_account(self, user_a, user_b, account_b):
        ...
        assert response.status_code in [403, 404], f"User A accessed User B's account! Status: {response.status_code}"
...
class TestNegativeValue:
    ...
    def test_negative_transfer_is_rejected(self, user_a, account_a, staff_user):
        ...
        response = client.post(
            "/api/transactions/process/",
            {"member_id": user_a.id, "amount": "-1000.00", "type": "deposit", "account_type": "daily_susu"},
        )

        # ASSERTION: Should be rejected with 400
        assert response.status_code == 400, f"Negative amount accepted! Status: {response.status_code}"
    ...
    def test_zero_amount_is_rejected(self, user_a, account_a, staff_user):
        ...
        response = client.post(
            "/api/transactions/process/",
            {"member_id": user_a.id, "amount": "0.00", "type": "deposit", "account_type": "daily_susu"},
        )

        assert response.status_code == 400, f"Zero amount accepted! Status: {response.status_code}"
