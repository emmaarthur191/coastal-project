...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            admin_user.set_password(admin_password)
            admin_user.save()
            self.stdout.write(self.style.SUCCESS("Admin user password updated from environment."))
        else:
            self.stdout.write("Admin user exists and password matches/not provided.")

        # Create or Update Manager
        manager_user, created = User.objects.get_or_create(
            email=manager_email, defaults={"username": manager_email, "role": "manager", "is_staff": True}
        )
        ...
        if created:
            ...
            manager_user.set_password(manager_password)
            manager_user.save()
            self.stdout.write(self.style.SUCCESS("Manager user password updated from environment."))
        else:
