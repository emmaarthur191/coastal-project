...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        staff_without_id = User.objects.filter(role__in=staff_roles, staff_id__isnull=True) | User.objects.filter(
            role__in=staff_roles, staff_id=""
        )
        ...
        for user in staff_without_id:
            try:
                with transaction.atomic():
                    ...
                    generated_count += 1
                    self.stdout.write(self.style.SUCCESS(f"Generated {staff_id} for {user.email}"))

            except Exception as e:
                errors.append(f"{user.email}: {e!s}")
                self.stdout.write(self.style.ERROR(f"Failed to generate ID for {user.email}: {e}"))
