...
from core.models import Account, Transaction, Expense
...
def verify_ids_and_expenses():
    ...
    try:
        ...
        if account.account_number.startswith('2231') and len(account.account_number) == 13:
            ...
            print(f"FAILURE: Account Number format incorrect: {account.account_number} (Len: {len(account.account_number)})")
    ...
    try:
        ...
        if expense:
            ...
            print(f"Expense Details: Amount={expense.amount}, Category={expense.category}, Desc='{expense.description}'")
        else:
