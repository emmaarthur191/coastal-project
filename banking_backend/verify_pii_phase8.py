...
def verify_pii_phase8():
    ...
    user, _ = User.objects.get_or_create(
        username="test_pii_user_p8", defaults={"email": "pii_p8@example.com", "role": "customer"}
    )
    manager, _ = User.objects.get_or_create(
        username="test_manager_p8", defaults={"email": "manager_p8@example.com", "role": "manager"}
    )
    cashier, _ = User.objects.get_or_create(
        username="test_cashier_p8", defaults={"email": "cashier_p8@example.com", "role": "cashier"}
    )
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT id_number_encrypted, next_of_kin_1_name_encrypted FROM core_loan WHERE id = %s", [loan.id]
        )
        row = cursor.fetchone()
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT occupation_encrypted, photo_encrypted FROM core_accountopeningrequest WHERE id = %s", [aor.id]
        )
        row = cursor.fetchone()
        assert row[0].startswith("gAAAAA")
    ...
    try:
        ca = ClientAssignment.objects.create(
            mobile_banker=manager, client=user, client_name="Bob Brown", location="Accra Mall"
        )
