...
class EmojiRemover {
    ...
    walkDirectory(dirPath, excludeDirs = null) {
        const files = [];
        if (excludeDirs === null) {
            excludeDirs = new Set();
        }
    ...
    processAllFiles() {
        console.log(' Starting comprehensive emoji removal process...');
...
`;
            ...
            for (const fileInfo of this.report.files_processed) {
                const statusIcon = fileInfo.syntax_valid ? '' : '';
                const statusClass = fileInfo.syntax_valid ? 'success' : 'error';
                const backupStatus = fileInfo.backup_created ? '' : '';
...
`;
            ...
            report += '<p>No emojis were found in any files. All files are already clean! </p>';
        }
...
`;
        ...
        if (this.report.total_emojis_removed > 0) {
            report += '<span class="success"> Completed - Emojis successfully removed and backups created</span></p>';
        } else {
            report += '<span class="success"> Completed - No emojis found in any files</span></p>';
        }
...
// Main execution
function main() {
    console.log(' Starting Comprehensive Emoji Removal Tool (Node.js Version)');
    ...
    try {
        ...
        console.log(` Total emojis removed: ${remover.report.total_emojis_removed}`);
        console.log(` Total files processed: ${remover.report.total_files_processed}`);
        console.log(` Backup files created: ${remover.report.backup_files_created}`);
        console.log(` Reports generated: EMOJI_REMOVAL_REPORT.html & .json`);
        console.log('='.repeat(60));
