...
import { LogOut, Menu, X, LayoutDashboard, Calendar } from 'lucide-react';
...
export default function DashboardLayout({
  ...
  const displayName =
    user?.name || (user?.first_name ? `${user.first_name} ${user.last_name || ''}` : 'User');
    ...
    <div className="min-h-[100dvh] flex bg-transparent">
      ...
      <aside className="hidden lg:flex flex-col w-64 glass-card-global fixed h-[96%] left-2 top-[2%] z-10 transition-all duration-300 border-r border-white/10 rounded-2xl shadow-xl">
        <div className="p-6 border-b border-white/10 flex items-center justify-center">
          ...
          <span className="ml-3 text-xl font-black tracking-tighter text-slate-900 drop-shadow-sm font-display uppercase">
            Coastal
          </span>
        </div>
        ...
        <div className="flex items-center p-4 bg-white/5 dark:bg-black/20 mx-4 mt-6 rounded-xl border border-white/10 backdrop-blur-sm">
          <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-primary-500 to-primary-300 flex items-center justify-center text-white font-bold border-2 border-white/20 shadow-sm relative overflow-hidden">
            {user?.profile_photo ? (
              <img
          ...
          <div className="ml-3 overflow-hidden">
            <p className="text-sm font-black truncate text-slate-900 uppercase tracking-tight">
              {displayName}
            </p>
            <p className="text-[10px] text-slate-900 font-black uppercase tracking-widest opacity-80 truncate">
              {roleDisplay}
            </p>
          </div>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-4 py-3 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all duration-200 group relative ${
                  isActive
                    ? 'bg-gradient-to-r from-primary-600 to-primary-700 text-white dark:text-slate-950 shadow-lg shadow-primary-500/40 dark:shadow-amber-500/20'
                    : 'text-slate-700 dark:text-slate-300 hover:bg-slate-900/5 dark:hover:bg-slate-800/40 hover:text-primary-600 dark:hover:text-amber-500'
                } ${item.available === false ? 'opacity-30 cursor-not-allowed' : ''}`}
              >
                <span
                  className={`flex items-center justify-center w-5 h-5 mr-3 transition-colors ${isActive ? 'text-white dark:text-slate-950' : 'text-slate-400 group-hover:text-primary-500'}`}
                >
                  {item.icon}
                </span>
                {item.name}
                {isActive && (
                  <div className="absolute right-3 w-1.5 h-1.5 bg-white dark:bg-slate-950 rounded-full shadow-sm animate-pulse-gentle" />
                )}
              </button>
            );
          })}
      ...
      <div className="flex-1 flex flex-col lg:ml-[19rem] transition-all duration-300 min-h-[100dvh]">
        ...
        <main className="flex-1 p-4 lg:p-8 max-w-7xl w-full mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-black drop-shadow-sm flex items-center gap-2 text-slate-900 dark:text-white">
                <span className="text-4xl text-primary-500 dark:text-primary-400">
          ...
          <div className="animate-fade-in relative z-0">{children}</div>
        </main>
      </div>
        ...
        <div className="fixed inset-0 flex">
          <Dialog.Panel className="relative mr-16 flex w-full max-w-xs flex-1 flex-col glass-card-global pt-5 pb-4 border-r border-white/20">
            <div className="flex items-center justify-between px-4">
              <div className="flex items-center">
                <span className="ml-2 font-black text-2xl text-slate-900 dark:text-white">
                  Coastal
                </span>
              </div>
              <button
              ...
              <nav className="space-y-2">
                {menuItems.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center px-4 py-3 text-lg font-bold rounded-xl text-slate-900 dark:text-white ${
                      activeView === item.id
                        ? 'bg-primary-500 text-white dark:text-slate-950 shadow-lg dark:shadow-amber-500/20'
                        : 'hover:bg-white/10'
