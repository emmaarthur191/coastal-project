...
const ReportsSection: React.FC = () => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true);
      const response = await api.get<PaginatedResponse<Report> | Report[]>('reports/reports/');
      ...
      <GlassCard className="p-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            <label htmlFor="report-template-select" className="block text-sm font-semibold text-gray-700 mb-1 ml-1">Report Template</label>
            <select
          ...
          <div className="w-full md:w-48">
            <label htmlFor="report-format-select" className="block text-sm font-semibold text-gray-700 mb-1 ml-1">Format</label>
            <select
              ...
              <div key={report.id} className="p-6 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-gray-500 text-xs font-mono">
                      {(report.type || 'REPORT').toUpperCase()} • {report.generated_at ? new Date(report.generated_at).toLocaleString() : 'Date N/A'}
                    </p>
                  </div>
                            ...
                            <tr key={i} className="hover:bg-gray-50/50">
                              {Object.values(row).map((value, j) => (
                                <td key={j} className="px-4 py-2 whitespace-nowrap text-gray-600">
                                  {typeof value === 'number' && typeof value === 'number' ? formatCurrencyGHS(value) : String(value ?? '')}
                                </td>
                              ))}
                            </tr>
                          ))}
                ...
                <div className="flex justify-end">
                  <Button
                    ...
                    size="sm"
                  >
                    Download {(report.format || 'PDF').toUpperCase()} 📥
                  </Button>
                </div>
              </div>
            ))}
