...
import { Building2, FileText, History, PlusCircle, Download } from 'lucide-react';
...
const RequestServicesTab: React.FC<RequestServicesTabProps> = ({
  ...
  const handleStatementRequest = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post<{ success: boolean; error?: string }>(
        'operations/statements/request_statement/',
        {
      ...
      if (response.data?.success) {
        setMessage({
          type: 'success',
          text: 'Statement generated successfully! You can download it from history.',
        });
  ...
  const handleCheckbookRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Checkbook request submitted! Awaiting Operations Manager approval.',
      });
    ...
    <div>
      ...
      {activeTab === 'statement' && (
        <div className="form-card">
          ...
          <p className="form-card-description">
            Generate a PDF statement of your account activity for a specific period. The statement
            will be generated instantly based on your transaction history.
          </p>
          <form onSubmit={handleStatementRequest}>
            ...
            >
              ...
              {accounts.map((acc) => (
                <option key={acc.id} value={acc.id}>
                  {acc.account_number} - {acc.account_type_display || acc.account_type} (GHS{' '}
                  {Number(acc.calculated_balance ?? acc.balance ?? 0).toFixed(2)})
                </option>
              ))}
            </select>
            ...
            <div className="date-grid">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setStatementForm({ ...statementForm, start_date: e.target.value })
                  }
                  className="request-service-input"
                />
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="request-service-button flex items-center justify-center gap-2"
            >
              {loading ? (
                'Generating...'
              ) : (
      ...
      {activeTab === 'checkbook' && (
        <div className="form-card">
          ...
          <p className="form-card-description">
            Submit a request for a new checkbook. Your request will be reviewed by the Operations
            Manager.
          </p>
          <form onSubmit={handleCheckbookRequest}>
            ...
            <label className="form-label">Number of Checkbooks</label>
            <select
              ...
              {serviceRequests.length === 0 ? (
                ...
                serviceRequests.map((req) => (
                  <tr key={req.id}>
                    ...
                    <td className="meta-cell">{new Date(req.created_at).toLocaleDateString()}</td>
                    <td className="meta-cell">{req.admin_notes || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
