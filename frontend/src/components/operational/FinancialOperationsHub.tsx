...
const FinancialOperationsHub: React.FC<FinancialOperationsHubProps> = ({
    ...
    try {
      ...
      if (res.success && res.data) {
        const data = Array.isArray(res.data)
          ? res.data
          : (res.data as { results?: StaffMember[] }).results;
        setStaffMembers(Array.isArray(data) ? (data as unknown as StaffMember[]) : []);
      }
    } finally {
      setLoading(false);
    }
      ...
      <GlassCard className="p-0 overflow-hidden border border-black/5">
        ...
        <div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
          {expenses.filter((ex) => ex.status === 'pending').length === 0 ? (
            <div className="p-8 text-center text-slate-900 font-black uppercase text-[10px] tracking-widest bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl">
            ...
            expenses
              ...
              .map((ex, i) => (
                <div
                  key={i}
                  className="p-4 flex justify-between items-center hover:bg-black/5 transition-all group border-b border-black/5 last:border-0"
                >
                  <div>
                  ...
                  <div className="font-black text-red-700 bg-red-500/10 px-3 py-1 rounded-full border border-red-500/10">
                    GHS {Number(ex.amount).toLocaleString()}
                  </div>
                </div>
              ))
          )}
          {expenses.length === 0 && (
            <div className="p-12 text-center text-slate-900 font-black uppercase text-[10px] tracking-widest bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl">
  ...
  const renderPayroll = () => (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <GlassCard className="p-8 border-t-[8px] border-t-emerald-500">
        ...
        <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-8">
          Authorize official employee payslips with full earnings and deductions.
        </p>

        <form className="space-y-6" onSubmit={handleGeneratePayslip}>
          <div>
            <label className="block text-[10px] font-black text-slate-900 uppercase mb-2 ml-1 tracking-widest">
              Select Staff Member
            </label>
            <select
          ...
          <div className="bg-slate-900 text-white p-6 rounded-2xl space-y-3 shadow-2xl">
            ...
            <div className="flex justify-between items-center text-red-400">
              ...
              <span className="text-xs font-bold">
                -
                {(Number(payrollData.base_pay) * 0.055).toLocaleString(undefined, {
            ...
            <div className="flex justify-between items-center text-red-400">
              ...
              <span className="text-xs font-bold">
                -
                {(
                  Number(payrollData.tax_deduction) + Number(payrollData.other_deductions)
                ).toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex justify-between items-center pt-2 border-t border-white/20">
              <span className="font-black text-emerald-400 uppercase tracking-tighter">
                Net Take Home
              </span>
              <span className="text-2xl font-black text-emerald-400">
                {(
                  ...
                  (Number(payrollData.base_pay) * 0.055 +
                    ...
                    (Number(payrollData.other_deductions) || 0))
                ).toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </span>
            </div>
          </div>
      ...
      <GlassCard className="p-0 overflow-hidden border border-black/5">
        <div className="p-4 border-b border-black/5 bg-black/5 font-black text-[10px] text-slate-900 uppercase tracking-[0.2em] ml-1">
        ...
        <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
          {Array.isArray(staffMembers) &&
            staffMembers.map((s) => (
              <div
                key={s.id}
                className="p-4 flex items-center justify-between hover:bg-emerald-500/5 transition-all cursor-pointer group border-b border-black/5 last:border-0"
                onClick={() => setPayrollData({ ...payrollData, staff_id: String(s.id) })}
              >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="text-emerald-700 font-black opacity-0 group-hover:opacity-100 scale-90 bg-emerald-500/10 border border-emerald-500/20"
                >
                  Quick Pay
                </Button>
              </div>
  ...
  const renderCashFlow = () => {
    if (!cashFlow)
      return <div className="text-center p-12 text-gray-400">No cash flow data available.</div>;
        ...
        <GlassCard className="flex justify-between items-center p-8 border border-black/5">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter">
              GH₵
              {Math.abs(cashFlow.net_cash_flow).toLocaleString(undefined, {
                minimumFractionDigits: 2,
              })}
            </h2>
            <div className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] mt-1 ml-1">
              Net Cash Flow ({cashFlow.period})
            </div>
          </div>
          <div
            className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border ${cashFlow.net_cash_flow >= 0 ? 'bg-emerald-500/10 text-emerald-800 border-emerald-500/20' : 'bg-red-500/10 text-red-800 border-red-500/20'}`}
          >
  ...
  const renderAnalysis = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <GlassCard className="p-8 border-t-[6px] border-t-amber-500">
        ...
        <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-6">
          Calculate estimated interest payouts for all active savings accounts.
        </p>
          ...
          <div className="mb-6 p-4 bg-amber-500/5 rounded-xl border border-amber-500/10">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-xl font-black text-amber-600">
                GH₵{analysis.interest_projection.estimated_monthly_payout.toLocaleString()}
              </span>
            </div>
            <div className="text-[10px] text-slate-600 font-black italic">
              Based on GH₵{analysis.interest_projection.total_savings_base.toLocaleString()} total
              liquid assets
            </div>
          </div>
        )}
      ...
      <GlassCard className="p-8 border-t-[6px] border-t-purple-500">
        ...
        <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-6">
          Verify and process commission shares for active mobile banking agents.
        </p>
          ...
          <div className="mb-6 p-4 bg-purple-500/5 rounded-xl border border-purple-500/10">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-xl font-black text-purple-600">
                GH₵{analysis.commissions.accrued_commissions.toLocaleString()}
              </span>
            </div>
            <div className="text-[10px] text-slate-600 font-black italic">
              Calculated from GH₵{analysis.commissions.monthly_volume.toLocaleString()} deposit
              volume
            </div>
          </div>
        )}
  ...
  const renderStatements = () => (
    <GlassCard className="p-8 max-w-2xl mx-auto border-t-[8px] border-t-blue-500">
      <h3 className="text-xl font-black text-slate-900 mb-2 font-display">
        System Audit Statements
      </h3>
      <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-8 text-center px-4 italic leading-relaxed">
        Internal audit-grade statement generation for system-wide reconciliation.
      </p>
      <form className="space-y-6" onSubmit={handleGenerateStatement}>
        <Input
          label="Target Account / System ID"
          value={statementData.account_number}
          onChange={(e) => setStatementData({ ...statementData, account_number: e.target.value })}
        />
        <div className="grid grid-cols-2 gap-4">
          <Input
            label="Start Date"
        ...
        <Button
          ...
          variant="primary"
          disabled={isSubmitting}
          className="w-full py-4 text-lg shadow-xl shadow-blue-100 flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            'Generating...'
          ) : (
  ...
  const renderContent = () => {
    if (loading && !expenses.length && !cashFlow)
      return (
        <div className="p-12 text-center text-slate-900 font-black uppercase text-[11px] tracking-[0.4em] animate-pulse">
          Synchronizing Banking Matrix...
        </div>
      );
      ...
      <div className="flex flex-wrap gap-2">
        {visibleTabs.map((tab) => (
          <button
            ...
            onClick={() =>
              setActiveTab(
                tab.id as 'expenses' | 'payroll' | 'cash-flow' | 'analysis' | 'statements'
              )
            }
            ...
            className={`px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 border ${
              activeTab === tab.id
                ? 'bg-blue-600 text-white border-blue-600 dark:bg-amber-500 dark:text-slate-950 dark:border-amber-500 shadow-xl shadow-blue-500/20 dark:shadow-amber-500/20'
                : 'bg-white/70 dark:bg-slate-900/30 text-slate-600 dark:text-slate-300 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 border-slate-200/50 dark:border-slate-800/60'
            }`}
          >
