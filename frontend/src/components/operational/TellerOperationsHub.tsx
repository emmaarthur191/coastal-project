...
const TellerOperationsHub: React.FC<TellerOperationsHubProps> = ({
  mode,
  initialTab = 'deposit',
}) => {
  ...
  const processTransaction = async (e: React.FormEvent, type: 'Deposit' | 'Withdrawal') => {
    ...
    try {
      ...
      if (error instanceof AxiosError) {
        showToast(
          'error',
          error.response?.data?.message || error.response?.data?.error || `${type} failed`
        );
      } else {
  ...
  const processCheck = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!checkMemberId || !checkAmount || !frontImage) {
    ...
    try {
      ...
      const r = await api.post<{ transaction_id: string }>(
        'check-deposits/process-check-deposit/',
        formData
      );
      ...
      if (error instanceof AxiosError)
        showToast('error', error.response?.data?.error || 'Check deposit failed');
  ...
  const handleSearch = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const res = await api.get<{ results: Transaction[]; count: number }>(
  ...
  const openDrawerSubmit = async () => {
    if (!openingBalance || parseFloat(openingBalance) < 0)
      return showToast('error', 'Invalid balance');
    setLoading(true);
    try {
      ...
      showToast('error', 'Failed to open drawer');
    } finally {
      setLoading(false);
    }
  };
  ...
  const closeDrawerSubmit = async () => {
    ...
    try {
      ...
      showToast('error', 'Failed to close drawer');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {message.text && (
        <div
          className={`fixed top-24 right-8 px-6 py-4 rounded-xl shadow-xl z-50 text-white font-bold animate-in slide-in-from-right duration-300 flex items-center gap-3 ${message.type === 'error' ? 'bg-red-600' : 'bg-emerald-600'}`}
        >
          {message.type === 'error' ? (
      ...
      {activeTab === 'deposit' && (
        <GlassCard className="max-w-xl mx-auto p-8 border-t-[6px] border-t-primary-500">
          <h2 className="text-2xl font-black text-slate-900 mb-6 text-center tracking-tighter">
            New Deposit
          </h2>
          ...
          <form onSubmit={processCheck} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                {frontImage ? (
                  <span className="flex items-center justify-center gap-2 text-emerald-600">
                    Front Selected <CheckCircle2 className="w-4 h-4" />
                  </span>
                ) : (
                  'Upload Front Image'
                )}
              ...
              >
                {backImage ? (
                  <span className="flex items-center justify-center gap-2 text-emerald-600">
                    Back Selected <CheckCircle2 className="w-4 h-4" />
                  </span>
                ) : (
                  'Upload Back Image'
                )}
      ...
      {activeTab === 'search' && mode === 'cashier' && (
        <GlassCard>
          ...
          <Pagination
            ...
            totalItems={searchPagination.total_count}
            itemsPerPage={20}
        ...
        <GlassCard>
          <div className="flex justify-between items-center mb-6">
            ...
            <div>
              {!currentDrawer || currentDrawer.status !== 'open' ? (
                <Button variant="success" onClick={() => setShowOpenDrawer(true)}>
                  Open Drawer
                </Button>
              ) : (
                <Button variant="danger" onClick={() => setShowCloseDrawer(true)}>
                  Close Drawer
                </Button>
              )}
            </div>
          ...
          {currentDrawer && (
            <div className="grid grid-cols-2 gap-4 bg-blue-50 p-4 rounded mt-4">
              <div>
                Status: <strong>{currentDrawer.status}</strong>
              </div>
              <div>
                Current Balance: <strong>{formatCurrencyGHS(currentDrawer.current_balance)}</strong>
              </div>
            </div>
          )}
        </GlassCard>
      ...
      {activeTab === 'closure' && mode === 'cashier' && (
        <GlassCard className="max-w-2xl mx-auto p-8 border-t-[6px] border-t-accent-600">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-black text-slate-900 uppercase ml-1 mb-2">
            ...
            {foundAccount && (
              <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-2xl border border-blue-100 dark:border-blue-800 animate-in fade-in zoom-in duration-300">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] font-black text-blue-500 uppercase block mb-1">
                      Account Holder
                    </span>
                    <span className="text-lg font-bold text-slate-900 dark:text-white">
                      {foundAccount.user?.full_name ||
                        foundAccount.user?.first_name + ' ' + foundAccount.user?.last_name}
                    </span>
                  </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-900 uppercase ml-1">
                Reason for Closure
              </label>
              <textarea
            ...
            <p className="text-[10px] text-slate-400 text-center italic mt-2 flex items-center justify-center gap-1">
              <AlertTriangle className="w-3 h-3 text-red-500" /> Caution: This action is permanent
              and will settle all remaining balances before closure.
            </p>
          </div>
        </GlassCard>
      )}
