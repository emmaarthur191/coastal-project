  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const safeFetch = async <T,>(p: Promise<T>) =>
        p.catch((e) => ({ success: false, error: String(e), data: undefined }) as unknown as T);
      ...
      if (fraudRes.success) {
        ...
        setFraudAlerts(
          (Array.isArray(data)
            ? data
            : (data as { results?: ExtendedFraudAlert[] })?.results || []) as ExtendedFraudAlert[]
        );
      }
      if (reportsRes) {
        const res = reportsRes as {
          data?: { monthly_data?: unknown[]; category_data?: unknown[] };
        };
        setReportsData({
  ...
  const handleSelectThread = async (thread: MessageThreadExtended) => {
    ...
    if (res.success) {
      ...
      setMessages(
        (Array.isArray(data)
          ? data
          : (data as { results?: ExtendedMessage[] })?.results || []) as ExtendedMessage[]
      );
    }
  };
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
      ...
      case 'accounts':
        return (
          <ErrorBoundary>
            <AdministrativeHub initialTab="accounts" />
          </ErrorBoundary>
        );
      case 'messaging':
        return (
          <OperationalMessenger
            ...
            onSelectThread={
              handleSelectThread as unknown as React.ComponentProps<
                typeof OperationalMessenger
              >['onSelectThread']
            }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#F8FAFC] flex">
      ...
      <aside className="w-72 bg-white border-r border-slate-200 flex flex-col h-screen sticky top-0">
        <div className="p-8 border-b border-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-primary font-black text-slate-800 text-lg leading-none">
                Coastal
              </h1>
        ...
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto custom-scrollbar">
          {[
            ...
            >
              <span
                className={`transition-colors ${activeView === item.id ? 'text-emerald-600' : 'text-slate-400'}`}
              >
                {item.icon}
              </span>
              {item.label}
        ...
        <div className="p-6 border-t border-slate-50 bg-slate-50/50">
          <Button
            variant="danger"
            className="w-full h-12 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"
      ...
      <main className="flex-1 p-10 max-w-[1600px] mx-auto">
        ...
        <header className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">
              Secure Internal Terminal â€¢ Node: AMER-SE-01
            </p>
          </div>
        ...
        {notification && (
          <div
            className={`p-4 rounded-2xl mb-8 flex items-center justify-between shadow-lg animate-in fade-in slide-in-from-top-4 duration-300 ${
          ...
          >
            ...
            <button
              ...
              className="opacity-50 hover:opacity-100 font-black p-1"
