...
const CashierDashboard: React.FC = () => {
  ...
  const [initialView] = useState<'loans' | 'cash_advances' | 'refunds' | 'pending-loans'>(
    'cash_advances'
  );
  ...
  const tabs = [
    ...
    },
    {
      ...
      icon: <UserPlus2 className="w-full h-full" />,
    },
  ...
  const fetchTransactions = async () => {
    try {
      ...
      _setTransactions(r.data.results || []);
    } catch (e) {
      console.error(e);
    }
  };
      ...
      if (event.ctrlKey || event.metaKey) {
        switch (event.key.toLowerCase()) {
          case 'o':
            ...
            setActiveTab('teller_operations');
            break;
          case '/':
            event.preventDefault();
  ...
  const renderActiveTabContent = () => {
    switch (activeTab) {
      case 'teller_operations':
        return (
          <ErrorBoundary>
            <TellerOperationsHub mode="cashier" initialTab="deposit" />
          </ErrorBoundary>
        );
      ...
      case 'account_opening':
        return (
          <ErrorBoundary>
            <OnboardingHub mode="staff" />
          </ErrorBoundary>
        );
      case 'support':
        return (
          <ErrorBoundary>
            <SupportHub mode="staff" initialTab="complaints" />
          </ErrorBoundary>
        );
      case 'my_payslips':
        return (
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        {message.text && (
          <div
            className={`fixed top-24 right-8 px-6 py-4 rounded-2xl shadow-2xl z-50 text-white font-bold animate-in slide-in-from-right duration-300 flex items-center gap-3 ${message.type === 'error' ? 'bg-error-600' : 'bg-success-600'} `}
          >
            {message.type === 'error' ? (
