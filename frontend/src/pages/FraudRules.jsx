...
const FraudRules = () => {
  ...
  const fetchRules = async () => {
    try {
      const response = await api.get('/api/fraud/rules/');
  ...
  const fetchActiveRules = async () => {
    try {
      const response = await api.get('/api/fraud/rules/active_rules/');
  ...
  const handleCreateRule = async () => {
    try {
      const response = await api.post('/api/fraud/rules/', formData);
  ...
  const handleUpdateRule = async () => {
    try {
      const response = await api.put(`/api/fraud/rules/${editingRule.id}/`, formData);
  ...
  const handleToggleActive = async (ruleId) => {
    try {
      await api.post(`/api/fraud/rules/${ruleId}/toggle_active/`);
      fetchRules();
      fetchActiveRules();
    } catch (error) {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: 'bold', marginBottom: '8px', color: '#1e293b' }}>Fraud Detection Rules</h1>
